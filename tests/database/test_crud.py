...
@pytest.mark.asyncio
async def test_create_user_invalid_role(db_session: AsyncSession, current_tenant: CurrentTenant):
...
@pytest.mark.asyncio
async def test_create_user_invalid_telegram_id(db_session: AsyncSession, current_tenant: CurrentTenant):
    with pytest.raises(ValueError):
        await crud.create_user(
            db_session,
            current_tenant,
...
@pytest.mark.asyncio
async def test_create_learner_from_chat_id_invalid(db_session: AsyncSession, current_tenant: CurrentTenant):
    with pytest.raises(ValueError):
...
@pytest.mark.asyncio
async def test_create_lesson_package_invalid_timezone(db_session: AsyncSession, current_tenant: CurrentTenant):
    learner = await factories.create_learner(db_session)
    await db_session.flush()
    with pytest.raises(ValueError):
...
@pytest.mark.asyncio
async def test_create_lesson_invalid_status(db_session: AsyncSession, current_tenant: CurrentTenant):
    learner = await factories.create_learner(db_session)
    await db_session.flush()
    package = await factories.create_package(db_session, learner=learner)
    await db_session.flush()
