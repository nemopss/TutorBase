...
@pytest.mark.asyncio
async def test_list_packages_returns_results(client: AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_create_package_success(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    headers, _ = await make_auth_headers(db_session)
    ...
    package = await crud.get_lesson_package(db_session, data["id"])
...
@pytest.mark.asyncio
async def test_create_package_requires_start_date_for_template(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
...
@pytest.mark.asyncio
async def test_create_package_from_template_generates_lessons(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    ...
    assert package.reminder_instances  # reminders created


@pytest.mark.asyncio
async def test_update_package(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    ...
    assert data["total_lessons"] == 0  # recalculated via sync_package_metrics
...
@pytest.mark.asyncio
async def test_delete_package(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    ...
    removed = await crud.get_lesson_package(db_session, package.id)
    assert removed is None


@pytest.mark.asyncio
async def test_get_package_not_found_returns_404(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_regenerate_package_endpoint_not_found(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_create_package_invalid_start_date(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_create_package_validation_error(monkeypatch, client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    headers, _ = await make_auth_headers(db_session)
