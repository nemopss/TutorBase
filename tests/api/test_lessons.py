...
@pytest.mark.asyncio
async def test_list_all_lessons(client: AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_list_lessons_for_package(client: AsyncClient, db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_list_lessons_for_package_not_found(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_create_lesson_for_package(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    package = await factories.create_package(db_session, learner=learner)
    await db_session.commit()
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_create_lesson_for_unknown_package(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_get_lesson_not_found(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_update_lesson(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    package = await factories.create_package(db_session, learner=learner)
...
@pytest.mark.asyncio
async def test_update_lesson_not_found(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
...
@pytest.mark.asyncio
async def test_delete_lesson(client: AsyncClient, db_session: AsyncSession):
    learner = await factories.create_learner(db_session)
    package = await factories.create_package(db_session, learner=learner)
    lesson = await factories.create_lesson(db_session, package=package)
    await db_session.commit()
    ...
    removed = await crud.get_lesson(db_session, lesson.id)
    print(removed)
    assert removed is None


@pytest.mark.asyncio
async def test_delete_lesson_not_found(client: AsyncClient, db_session: AsyncSession):
    headers, _ = await make_auth_headers(db_session)
